...
switch (SWITCH) {
  case 0:
    appendToArhive(nameArchive, URL).then((archive) => {
      console.log('#########');
  ...
  case 1:
      readAndSend(nameArchive,SERVER).then((body) => {
        const stats = fs.statSync(path.join(path.resolve('./temp'), nameArchive));
        const fileSizeInBytes = stats.size;
        const res=JSON.parse(body);

        if (fileSizeInBytes===res.stats.size) {
          console.log('Файл успешно отправлен');
        } else{console.log('Что то пошло не так при отправке файла');}
      });
